...
use object_store::{
    ...
    Error::Generic,
    ObjectStore,
    aws::{AmazonS3Builder, AwsCredential, resolve_bucket_region},
    ...
    pub async fn try_new() -> Result<Self> {
        ...
        let credentials = match credentials.provide_credentials().await {
            ...
            Err(e) => {
                ...
                let message =
                    format!("Error getting credentials from provider: {e}{source_message}",);
...
pub(crate) async fn get_object_store(
    ...
    let store: Arc<dyn ObjectStore> = match scheme {
        "s3" => {
            ...
            let builder = get_s3_object_store_builder(url, options, resolve_region).await?;
